...
primitive_root :: (String,String) -> (String,String)
primitive_root (u,v) =  if odd (length u) then (u,v)
                        else checkpattern (u,v)


selective_repeat :: Int -> String -> String
selective_repeat 0 x = x
...
process_stack (T (Node "KS" (Just left) Nothing) : xs) = do
                                                            x <- process_stack [T left]
                                                            if x == False
                                                            then return False
                                                            else process_stack xs
